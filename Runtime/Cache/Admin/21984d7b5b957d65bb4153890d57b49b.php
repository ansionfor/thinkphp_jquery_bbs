...
<div class="container">
	<div class="row">
      ...
      <table class="table table-striped table-hover">
        ...
        <tbody>
        <?php if(is_array($tlist)): $i = 0; $__LIST__ = $tlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            <th scope="row"><?php echo ($vo["title"]); ?></th>
            <td>
            <?php switch($vo["type"]): case "2": ?>需求<?php break;?>
            <?php case "3": ?>服务<?php break; endswitch;?>
            </td>
            <td><a href="/web/Index/memberPub/mid/<?php echo ($vo["member_id"]); ?>"><?php echo ($vo["name"]); ?></a></td>
            <td>
            <?php switch($vo["state"]): case "1": ?>进行中<?php break;?>
            ...
            <td>
            <?php switch($vo["status"]): case "0": ?><a href="javascript:void(0);" onclick="passTask(<?php echo ($vo["task_id"]); ?>)">通过</a><?php break;?>
            <?php case "1": ?><a href="javascript:void(0);" onclick="notPassTask(<?php echo ($vo["task_id"]); ?>);">取消</a><?php break; endswitch;?>
            &nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" onclick="deleteTask(<?php echo ($vo["task_id"]); ?>);">删除</a></td>
...
<script>
  ...
  function notPassTask (tid) {
    $.ajax({
      url:"<?php echo U('Index/notPassTask');?>",
      type:'get',
      data:{
        "tid":tid
      },
  ...
  function deleteTask (tid) {
    $.ajax({
      url:"<?php echo U('Index/deleteTask');?>",
      type:'get',
      data:{
        "tid":tid
      },
